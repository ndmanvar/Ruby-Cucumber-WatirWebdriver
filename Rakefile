...
task :run do
	junitdir = ENV['JUNIT_DIR']
     CukeForker::Runner.run CukeForker::Scenarios.all,
                           :max => 8,
                           :extra_args => %W[-f CukeForker::Formatters::JunitScenarioFormatter --out junit/#{junitdir}]
end

def run_tests(platform, browser, version, junit_dir)
  system("platform=\"#{platform}\" browserName=\"#{browser}\" version=\"#{version}\" JUNIT_DIR=\"#{junit_dir}\" rake run")
end
